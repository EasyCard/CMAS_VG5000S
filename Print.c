...
void PrintDateTime(STR *Date,STR * Time)
{
   ...
   yy[4]=0;mm[2]=0;dd[2]=0;hh[2]=0;min[2]=0;ss[2]=0;
  memcpy(yy,&Date[0],4); memcpy(mm,&Date[4],2); memcpy(dd,&Date[6],2);
  memcpy(hh,&Time[0],2); memcpy(min,&Time[2],2); memcpy(ss,&Time[4],2);
...
void PrintTitle()
{
    ...
    memset(tmpbuf,0x00,sizeof(tmpbuf)); memset(tmpbuf2,0x00,sizeof(tmpbuf2));
