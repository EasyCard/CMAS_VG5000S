...
USHORT Function_Signon()
{
    ...
    BYTE Initflag=0;
    void *threadResult;
    unsigned long ltmp;unsigned int itmp;
 ...
 //   CTOS_Delay(500);
  ...
  //  if(ret!=d_OK) return ret;
    ...
    if(gScrpitTesting==1 || (ecrObj.ecrOn==TRUE && ecrObj.autoSignOn==TRUE)){/*no needed to check PWD*/}
    else{
...
USHORT Trans_Refund()
{
  ...
  //  usRet=pthread_create(&Thread_Comm, NULL, (void*)SSLSocketConnect_thread, NULL);    // 執行緒 SSLSocketConnect 
    STR line1[32],line2[32];int amt=0;
    memset(Buffer,0x00,sizeof(Buffer));
